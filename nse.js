...
import { saveDataToFile, readLastFile } from './ip.js';
...
async function getNseOptionChain(symbol = 'RELIANCE') {
  ...
  try {
    ...
    await client.get(`https://www.nseindia.com/get-quotes/derivatives?symbol=${symbol}`, {
      headers,
    });
  ...
  saveDataToFile(
    {
      symbol,
      timestamp: Date.now(),
      dateLocal: new Date().toLocaleTimeString(),
      date: new Date(),
      data: nseData,
    },
    symbol,
...
async function getSaveData(symbol = 'RELIANCE', strikePrice = 1200) {
  try {
    let getNse = await readLastFile(symbol);
    const filtered = optionData(getNse);
    // saveMultipleData(filtered);
    return {
      success: true,
...
async function getNiftyData() {
  ...
  try {
    ...
    const response = await client.get(`https://www.nseindia.com/api/marketStatus`, { headers });
...
export { getData, getNiftyData, saveOptionsCron, getSaveData };
